...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
    ...
    // copies an options.txt file placed inside dev/ to development environment client
    private fun Project.configureDevOptions() {
        tasks.register<Copy>("prepareDevOptions") {
            val source = rootProject.file("dev/options.txt")
            val target = file("run/options.txt")
        ...
        tasks.named("runClient").configure {
            dependsOn(tasks.named("prepareDevOptions"))
        }
    }
}
